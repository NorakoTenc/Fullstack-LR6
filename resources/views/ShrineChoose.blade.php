...
@endif
        ...
        <main class="mt-6">
        @if ($hasBeatrice)
        <p>Shrine of Beatrice exists in the database.</p>
    @else
        <p>Shrine of Beatrice does not exist in the database.</p>
    @endif
    @if ($hasErica)
        <p>Shrine of Erica exists in the database.</p>
    @else
        <p>Shrine of Erica does not exist in the database.</p>
    @endif
            <div class="grid gap-6 lg:grid-cols-2 lg:gap-8" id="aaaaaa2">
            @foreach ($shrines as $shrine)
            <a  href="{{ route('Shrine.show', $shrine->id) }}"
